...
@app.route('/siswa', methods=['GET','POST','PUT','DELETE'])
def mahasiswa():
    try: 
        ...
        # Jika Method POST
            ...
            usia = datainput['usia']
            query = "INSERT INTO data_siswa (nama, pekerjaan, usia) values (%s,%s,%s) "
        ...
        #Jika Method PUT
            ...
            query = "UPDATE data_siswa SET id = %s "
            datainput = request.json
            id_ = datainput['id']
            values += (id_,)
            ...
            # Selain itu adalah DELETE, bila ada method selain keempat ini maka dipastikan akan langsung error karena method tidak di assign.
                ...
                query = "DELETE FROM data_siswa id = %s "
                id_ = request.args.get("id")
                values = (id_,)
                dt.insert_data(query, values)
                data = [{
